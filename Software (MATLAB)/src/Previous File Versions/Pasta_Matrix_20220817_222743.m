...
%Collated: 08/17/2022, 22:27:41

Pasta_Matrix_Startup;                                                       %Call the startup function.
...
%% ***********************************************************************
function Pasta_Matrix_PopData_to_TSV(varargin)
%
%Pasta_Matrix_PopData_to_TSV.m - Vulintus, Inc., 2016.
%
%   PASTA_MATRIX_POPDATA_TO_TSV reads data from batches of pasta matrix 
%   records, in  the form of *.PNG files, computes user-selected task 
%   parameters, and then outputs the values into an Excel-readable TSV 
%   (tab-separated values) spreadsheet, with each line corresponding to one 
%   behavioral session.
%
%   UPDATE LOG:
%   02/16/2017 - Drew Sloan - Function first created.
%   08/17/2022 - Drew Sloan - Renamed to Pasta_Matrix_PopData_to_TSV.m.
%
...
if exist(filename,'file')                                                   %If a configuration file already exists...
    ...
    missing_fields = setdiff(fields.mandatory,output);                      %Check to see if any mandatory fields are missing.
    for i = 1:length(missing_fields)                                        %Step through each missing field.
        j = find(strcmpi(missing_fields{i},fields.mandatory));              %Find the mandatory column index for the missing field.
        output = vertcat(output(1:j-1), missing_fields(i), output(j:end));  %Add in the missing field.
    end
else                                                                        %Otherwise...
        ...
        for j = 1:length(output)                                            %Step through all of the output columns.                     
            switch output{j}                                                %Switch between the possible column values.
                ...
                case 'FARTHEST REACH (mm)'
                    ...
                    if ~isempty(temp)                                       %If there were any broken strands...
